...
**部署稳定性**
- 在 Docker Compose 配置中添加 `restart: unless-stopped` 策略，以便在宿主机重启或 Docker 守护进程重启时，服务可以自动启动恢复。
- 同步更新 README 和 Docker 部署文档中的相关配置说明。

### v1.3.4 (2026-05-21)

**提示与展示优化**
- 统一系统提示显示方式，有固定结果区的操作不再重复弹出全局 toast。
- 优化登录注册验证码提示，只在模态框结果区显示发送成功或失败信息。
- 后台 SMTP 测试、版本检查、套餐保存、会员操作和兑换码生成改为就近展示结果。

### v1.3.3 (2026-05-21)

**发信与诊断优化**
- 后台 SMTP 配置新增测试邮件按钮，可直接查看发送成功或失败详情。
- 优化验证码发信错误诊断，保留 SMTP 服务商原始响应并提示授权码/应用专用密码问题。
- 前台登录、注册、忘记密码和改邮箱模态框展示更完整的接口错误结果。

### v1.3.2 (2026-05-21)

**网络与部署**
- 新增 `Z7PDF_TRUST_PROXY` 配置，支持可信反向代理部署并避免登录限流误报代理头错误。
- 完善 Docker 部署说明，补充反向代理、管理员初始化和管理员密码找回流程。
- 明确普通注册用户不会自动获得管理员权限，降低公网首次部署时的误操作风险。

### v1.3.1 (2026-05-20)

**订阅与检查**
- 新增后台版本更新自动检查机制，支持自部署状态智能升级检测。
- 新增后台会员订阅管理与到期天数修改，完善套餐授权工作流。
- 优化后台概览数据卡片与版本卡片对齐，精细化时间戳视觉层级与防换行样式。
- 优化更新检查交互，无更新时采用静默处理，避免冗余弹窗提示。
